...
class MainApp extends PolymerElement {
  ...
  addTask(event) {
    ...
    this.$.modal.close();
    console.log("Reached debug point 1");
  }
  ...
  handleResponse(event, res) {
    if (this.$.dataAjax.method === "GET") {
      ...
      this.set("total_records", this.tasks.length);
      var i_done = 0;
      ...
      this.total_complete = i_done;
      this.percent_Complete =
        (this.total_complete * 100) / this.total_records + "%";
      this.updateStyles({
        "--custom-fillbar-width": this.percent_Complete
      });
    } else {
      this.getAll();
    }
